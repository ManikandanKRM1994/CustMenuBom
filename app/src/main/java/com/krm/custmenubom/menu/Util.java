...
public class Util {
    ...
    private static int getColor(View view, int id, Resources.Theme theme) {
    ...
    private static int getColor(TypedArray typedArray, int id, Resources.Theme theme) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    private static int getColor(TypedArray typedArray, int id) {
        return getColor(typedArray, id, null);
    }

    static Drawable getSystemDrawable(Context context, int id) {
    ...
    public static void setText(TextView textView, int id, String text) {
        ...
        if (id == 0) {
            if (text != null && !text.contentEquals(textView.getText())) textView.setText(text);
        } else {
    ...
    private static Drawable getDrawable(View view, int id) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    private static BitmapDrawable getOvalBitmapDrawable(View view, int radius, int color) {
        if (radius <= 0) {
            return null;
        }
    ...
    private static BitmapDrawable getRectangleBitmapDrawable(View view, int width, int height, int cornerRadius, int color) {
        if (width <= 0 || height <= 0) {
            return null;
        }
        Bitmap bitmap = Bitmap.createBitmap(
