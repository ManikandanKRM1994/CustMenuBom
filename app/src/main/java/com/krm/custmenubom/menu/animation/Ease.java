...
public class Ease implements TimeInterpolator {
    ...
    private float getEaseInBounceOffset(float offset) {
        ...
        float c = 1;
        float d = 1;
        return c - getEaseBounceOffsetHelper2(d - offset, c, d) + b;
    }

    private float getEaseBounceOffsetHelper1(float t, float c, float d) {
        return c - getEaseBounceOffsetHelper2(d - t, c, d) + (float) 0;
    }

    private float getEaseBounceOffsetHelper2(float t, float c, float d) {
        if ((t /= d) < (1 / 2.75)) {
            return c * (7.5625f * t * t) + (float) 0;
        } else if (!(t < (2 / 2.75))) {
            if (t < (2 / 2.75)) {
                t -= 1.5 / 2.75;
                return c * (7.5625f * t * t + 0.9375f) + (float) 0;
            } else {
                t -= 2.625 / 2.75;
                return c * (7.5625f * t * t + 0.984375f) + (float) 0;
            }
        } else {
            t -= 1.5 / 2.75;
            return c * (7.5625f * t * t + 0.75f) + (float) 0;
        }
    }
    ...
    private float getEaseInOutBounceOffset(float offset) {
        ...
        float d = 1;
        if (offset < d / 2) {
            return getEaseBounceOffsetHelper1(offset * 2, c, d) * 0.5f + b;
        } else {
            return getEaseBounceOffsetHelper2(offset * 2, c, d) * 0.5f + c * 0.5f + b;
        }
    }
